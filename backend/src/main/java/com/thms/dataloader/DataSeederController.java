...
public class DataSeederController {
    ...
    /**
     * Seeds the database with sample screenings - IMPROVED VERSION
     * Creates 2-3 screenings per UNIQUE movie for balanced distribution
     * @return ResponseEntity with the operation result
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> seedScreenings() {
        ...
        LocalTime[] times = {
                ...
                LocalTime.of(21, 30)  // 9:30 PM
        };

        // Formats for variety
        Screening.ScreeningFormat[] formats = {
                Screening.ScreeningFormat.STANDARD,
                Screening.ScreeningFormat.THREE_D,
                Screening.ScreeningFormat.IMAX
        };

        System.out.println("Starting screening seeding...");
        System.out.println("Found " + movies.size() + " movies and " + theatres.size() + " theatres");

        // Create 2-3 screenings per movie across different theatres and days
        int movieIndex = 0;
        int globalScreeningIndex = 0; // Track total screenings created

        for (MovieDTO movie : movies) {
            System.out.println("Processing movie: " + movie.getTitle() + " (ID: " + movie.getId() + ")");

            // Create 2-3 screenings for each movie
            int screeningsPerMovie = 2 + (movieIndex % 2); // Alternate between 2 and 3

            for (int i = 0; i < screeningsPerMovie; i++) {
                // Distribute across different theatres
                TheatreDTO theatre = theatres.get(i % theatres.size());

                // Skip if theatre doesn't have screens
                if (theatre.getTotalScreens() == null || theatre.getTotalScreens() < 1) {
                    System.out.println("Skipping theatre " + theatre.getName() + " - no screens");
                    skipped++;
                    continue;
                }

                // Spread across more days to avoid conflicts (0-6 days from today)
                int dayOffset = globalScreeningIndex % 7;
                LocalDate screeningDate = today.plusDays(dayOffset);

                // Use globalScreeningIndex to ensure unique time slots
                // This creates times like: 10:30, 11:00, 11:30, 12:00, 12:30, 13:00, etc.
                int hourOffset = (globalScreeningIndex / 2) % 12; // 0-11 hours offset
                int minuteOffset = (globalScreeningIndex % 2) * 30; // 0 or 30 minutes
                LocalTime screeningTime = LocalTime.of(10 + hourOffset, minuteOffset);

                // Vary screen numbers based on global index
                int screenNumber = (globalScreeningIndex % theatre.getTotalScreens()) + 1;

                // Vary formats
                Screening.ScreeningFormat format = formats[globalScreeningIndex % formats.length];
                ...
                try {
                    ScreeningDTO created = screeningService.createScreening(screening);
                    count++;
                    globalScreeningIndex++;
                    System.out.println("✓ Created screening " + count + ": " + movie.getTitle() +
                            " at " + theatre.getName() + " on " + screeningDate + " " + screeningTime +
                            " (Screen " + screenNumber + ")");
                } catch (Exception e) {
                    skipped++;
                    globalScreeningIndex++;
                    System.err.println("✗ Failed to create screening for " + movie.getTitle() +
                            " at " + theatre.getName() + " on " + screeningDate + " " + screeningTime +
                            ": " + e.getMessage());
                }
            }
        ...
        System.out.println("Screening seeding complete!");
        System.out.println("Created: " + count + ", Skipped: " + skipped);

        Map<String, Object> result = new HashMap<>();
        result.put("seeded", count);
        result.put("skipped", skipped);
        result.put("moviesProcessed", movies.size());
