...
public class TwoFactorAuthService {
    ...
    public boolean generateAndSendOtp(User user) {
        ...
        System.out.println("========================================");
        System.out.println("🔐 Generating OTP for user: " + user.getUsername());
        ...
        System.out.println("💾 OTP stored in memory for email: " + user.getEmail());

        // Send OTP via email
        boolean sent = sendOtpEmail(user.getEmail(), otp);

        if (sent) {
            System.out.println("✅ OTP generation and sending completed successfully");
        } else {
            System.out.println("❌ OTP generation succeeded but email sending failed");
        }
    ...
    public boolean verifyOtp(String email, String otpToVerify) {
        ...
        System.out.println("🔢 OTP to verify: " + otpToVerify);

        OtpData otpData = otpStore.get(email);

        if (otpData == null) {
            System.out.println("❌ No OTP found in store for this email");
            System.out.println("📋 Current OTP store keys: " + otpStore.keySet());
            System.out.println("========================================");
            return false; // No OTP found for this email
        }

        System.out.println("✅ OTP found in store");
        System.out.println("💾 Stored OTP: " + otpData.otp);
        System.out.println("⏰ Stored expiry: " + otpData.expiryTime);
        ...
        if (isValid) {
            ...
            System.out.println("🗑️ OTP removed from store (one-time use)");
        } else {
            System.out.println("❌ OTP does not match");
            System.out.println("Expected: " + otpData.otp);
            System.out.println("Received: " + otpToVerify);
        }
    ...
    private boolean sendOtpEmail(String email, String otp) {
        try {
            ...
            System.err.println("📧 Target email: " + email);
